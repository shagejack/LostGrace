...
public final class LostGraceConfig {
    ...
    static {
        BUILDER.push("Config for Lost Grace");

        GRACE_FORCE_FIRST_PERSON_DISTANCE = BUILDER.comment("Player within this distance of graces will be forcibly changed to first person. This distance is also used to determine if a grace should be rendered in fog. Default: 5.5").define("Grace Force First Person Distance", 5.5D);
        GRACE_MAX_DISTANCE = BUILDER.comment("The max distance allowed to interact with grace and to render fog. Default: 3.0").define("Grace Max Distance", 3.0D);
